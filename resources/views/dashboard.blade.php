...
<head>
    ...
    <title>Fjellserver.no | Status-side Dashboard</title>
...
<div class="container">
  ...
  @foreach($hosts as $key => $data)
    <ul class="list-group">
      <li class="list-group-item"><h5>{{$data->name}}</h5><p>{{$data->created_at}}</p><p>{{$data->ip}}</p><p>Port: {{$data->port}}</p><p>Rank: {{$data->rank}}</p><form action="{{url('dashboard')}}/host/{{$data->rank}}" method="post"> @csrf <button type="submit" class="btn btn-primary">Fjern</button> </form></li>
    </ul>
  @endforeach
  </div>
